...
function renderOccVisibility(){const root=$("occasionVisibilityList");if(!root)return;root.innerHTML=occasionOrder.map((o,i)=>`<label class="toggle-row"><input type="checkbox" data-occ="${o}" ${visibleOccasionTypes.includes(o)?"checked":""}/> <span>${o}</span><button type="button" class="mini" data-occ-up="${o}" ${i===0?"disabled":""}>↑</button><button type="button" class="mini" data-occ-down="${o}" ${i===occasionOrder.length-1?"disabled":""}>↓</button></label>`).join("");}
...
on('occasionVisibilityList','click',e=>{const up=e.target.dataset.occUp,down=e.target.dataset.occDown;if(up){const i=occasionOrder.indexOf(up);if(i>0){[occasionOrder[i-1],occasionOrder[i]]=[occasionOrder[i],occasionOrder[i-1]];}}if(down){const i=occasionOrder.indexOf(down);if(i>-1&&i<occasionOrder.length-1){[occasionOrder[i+1],occasionOrder[i]]=[occasionOrder[i],occasionOrder[i+1]];}}renderOcc([...$('occasionSelect').selectedOptions].map(o=>o.value));});
on('occasionVisibilityList','change',e=>{const occ=e.target.dataset.occ;if(!occ)return;visibleOccasionTypes=e.target.checked?Array.from(new Set([...visibleOccasionTypes,occ])):visibleOccasionTypes.filter(o=>o!==occ);renderOcc([...$('occasionSelect').selectedOptions].map(o=>o.value));});
