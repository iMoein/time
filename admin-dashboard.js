...
function updateSunBand(){const now=new Date();const utcHour=now.getUTCHours()+now.getUTCMinutes()/60;const subSolarLon=(12-utcHour)*15;const leftX=((subSolarLon+180)/360)*1000;const d=`M ${leftX-250} 0 C ${leftX-120} 120 ${leftX-120} 300 ${leftX-250} 420 L 1000 420 L 1000 0 Z`;const term=$("terminator");if(term)term.setAttribute('d',d);const sun=$("sunStatus");if(sun)sun.textContent=`Sun over UTC: ${now.toUTCString().slice(17,22)}`;}
