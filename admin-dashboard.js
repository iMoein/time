...
function renderNtp(){ const preset=$('ntpPreset'); if(!preset)return; preset.innerHTML=NTP.map(([h,l])=>`<option value='${h}'>${l} (${h})</option>`).join(''); const chips=$('ntpQuickServers'); if(chips){chips.innerHTML=NTP.filter(([h])=>h!=='custom').slice(0,8).map(([h,l])=>`<button type="button" class="ntp-chip" data-ntp="${h}" title="${h}">${l}</button>`).join('');}}
...
on('ntpPreset','change',e=>{if(e.target.value!=='custom'&&$('ntpHost'))$('ntpHost').value=e.target.value;const h=$('ntpHost')?.value||'-';if($('ntpHostLabel'))$('ntpHostLabel').textContent=h;});
on('ntpQuickServers','click',e=>{const btn=e.target.closest('[data-ntp]');if(!btn)return;const host=btn.dataset.ntp;if($('ntpHost'))$('ntpHost').value=host;if($('ntpPreset'))$('ntpPreset').value=host;if($('ntpHostLabel'))$('ntpHostLabel').textContent=host;});
