...
> The public clock page is static and can run on GitHub Pages. When the Node admin API is unavailable, open `/admin-login.html`, enter the repository connection details, and use **GitHub Pages Data Mode** with a fine-grained GitHub token that has Contents read/write access to this repository. Changes are committed through the GitHub API and become public after the Pages deploy workflow finishes. NTP probing still requires the Node server, so Pages uses the browser clock fallback.
