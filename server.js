...
function ensureConfig(){if(!existsSync(configPath)){writeFileSync(configPath,JSON.stringify({ntpHost:'pool.ntp.org',adminUsername:defaultAdminUser,adminPassword:defaultAdminPassword,passwordChanged:false,defaultCityIds:['tehran','los-angeles','london'],defaultSelectedCityId:'tehran',defaultOccasionTypes:['iran','iranCurrent','international','globalOfficial','islamic','islamicShia','islamicShared']},null,2));}const cfg=JSON.parse(readFileSync(configPath,'utf8'));return {ntpHost:cfg.ntpHost||'pool.ntp.org',adminUsername:cfg.adminUsername||defaultAdminUser,adminPassword:cfg.adminPassword||defaultAdminPassword,passwordChanged:Boolean(cfg.passwordChanged),defaultCityIds:Array.isArray(cfg.defaultCityIds)?cfg.defaultCityIds:['tehran','los-angeles','london'],defaultSelectedCityId:cfg.defaultSelectedCityId||'tehran',defaultOccasionTypes:Array.isArray(cfg.defaultOccasionTypes)?cfg.defaultOccasionTypes:['iran','iranCurrent','international','globalOfficial','islamic','islamicShia','islamicShared']};}
...
if(requestUrl.pathname==='/api/admin/config'&&request.method==='GET'){const cfg=ensureConfig();return sendJson(response,200,{ntpHost:cfg.ntpHost,defaultCityIds:cfg.defaultCityIds,defaultSelectedCityId:cfg.defaultSelectedCityId,defaultOccasionTypes:cfg.defaultOccasionTypes});}
if(requestUrl.pathname==='/api/admin/config'&&request.method==='POST'){const body=await readBody(request);const host=sanitizeNtpHost(body.ntpHost||'');if(!host) return sendJson(response,400,{error:'Invalid NTP host'});const cfg=ensureConfig();const cityIds=Array.isArray(body.defaultCityIds)?body.defaultCityIds.filter((v)=>typeof v==='string'&&v.trim()):cfg.defaultCityIds;const selected=typeof body.defaultSelectedCityId==='string'&&body.defaultSelectedCityId?body.defaultSelectedCityId:cfg.defaultSelectedCityId;const occasionTypes=Array.isArray(body.defaultOccasionTypes)?body.defaultOccasionTypes.filter((v)=>typeof v==='string'&&v.trim()):cfg.defaultOccasionTypes;writeConfig({...cfg,ntpHost:host,defaultCityIds:cityIds,defaultSelectedCityId:selected,defaultOccasionTypes:occasionTypes});return sendJson(response,200,{ok:true});}
